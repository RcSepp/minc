...
EXTERN_FUNC_TPLT = 'llvm_c_functions.push_back(Func("{name}", {builtin_restype}, {{ {builtin_argtypes} }}, false, "{exname}"));'
PAWS_EXTERN_FUNC_TPLT = 'defineExpr(pkgScope, "{name}({paws_argtypes})", +[] ({c_args}) -> {c_restype} {{ return {exname}({c_argvals}); }});'
...
for filedir, subdirs, filenames in os.walk('./templates'):
    for filename in filenames:
        ...
        def repl(match):
            ...
            if template_name == "LLVM_EXTERN_FUNC_DEF" or template_name == "PAWS_LLVM_EXTERN_FUNC_DEF" or template_name == "ACC_LLVM_EXTERN_FUNC_DEF":
                ...
                if template_name.startswith("PAWS_"):
                    ...
                    extern_func_tplt = EXTERN_FUNC_TPLT
                ...
                for decl in decls.values():
                    ...
                    # Convert c types to builtin types
                    try:
                        builtin_restype = get_builtin_type(decl.restype)
                        builtin_argtypes = [get_builtin_type(typestr) for typestr, c_arg in zip(decl.argtypes, decl.c_args) if c_arg not in REPLACE_PARAMS]
                    except UnknownTypeException as err:
                        unknown_types.add(err.typestr)
                        continue
                    ...
                    output.append(
                        ...
                        .format(
                            ...
                            paws_restype=paws_restype,
                            paws_argtypes=", ".join(paws_argtypes),
                            builtin_restype=builtin_restype,
                            builtin_argtypes=", ".join(builtin_argtypes),
