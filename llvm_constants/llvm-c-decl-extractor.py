...
EXTERN_FUNC_TPLT = 'llvm_c_functions.push_back(Func("{name}", {restype}, {{ {argtypes} }}, false, "{exname}"));'
PAWS_EXTERN_FUNC_TPLT = 'defineExpr(pkgScope, "{name}({argtypes})", +[] ({c_args}) -> {c_restype} {{ return {exname}({c_argvals}); }});'
...
for filename in os.listdir("./templates"):
    ...
    def repl(match):
        ...
        if template_name == "LLVM_EXTERN_FUNC_DEF" or template_name == "PAWS_LLVM_EXTERN_FUNC_DEF":
            ...
            output = []
            unknown_types = set()
            get_type = get_paws_type if template_name.startswith("PAWS_") else get_builtin_type
            extern_func_tplt = PAWS_EXTERN_FUNC_TPLT if template_name.startswith("PAWS_") else EXTERN_FUNC_TPLT
            modified_prefix = "PAWSLLVM" if template_name.startswith("PAWS_") else "LLVMEX"
            for decl in decls.values():
                if decl.name == 'LLVMConstInt':
                    abc = 0
                try:
                    restype = get_type(decl.restype)
                    argtypes = [get_type(typestr) for typestr, c_arg in zip(decl.argtypes, decl.c_args) if c_arg not in REPLACE_PARAMS]
                ...
                output.append(
                    ...
                    .format(
                        ...
                        restype=restype,
                        c_args=", ".join((c_arg for c_arg in decl.c_args if c_arg not in REPLACE_PARAMS)),
                        c_argvals=", ".join((c_arg[len(c_argtype):].strip(' []') for c_arg, c_argtype in zip(decl.c_args, decl.c_argtypes) if c_arg not in REPLACE_PARAMS)),
                        argtypes=", ".join(argtypes),
                        exname=(modified_prefix + decl.name[4:]) if decl.is_modified else decl.name
                    )
                )
        ...
        elif template_name == "MODIFIED_LLVM_EXTERN_FUNC_DEF" or template_name == "PAWS_MODIFIED_LLVM_EXTERN_FUNC_DEF":
