...
MincRuntime::MincRuntime(MincBlockExpr* parentBlock, bool resume, MincInteropData& interopData)
	: parentBlock(parentBlock), resume(resume), stackFrames(new MincStackFrame[1024]), heapFrame(nullptr), currentStackPointerIndex(0), stackSize(STACK_SIZE), stack(new unsigned char[stackSize]), currentStackSize(0), interopData(interopData)
{
}
