...
%language "C++"
%locations
%define api.parser.class {GoParser}
%define api.value.type variant
%parse-param {GoLexer& scanner}
...
%token NEWLINE // Language specific tokens
...
stmt_string
	...
	| stmt_string NEWLINE { $$ = $1; } // Eat NEWLINE-terminated empty statements
...
stmt
	...
	| stmt_expr_list NEWLINE { ($$ = $1)->push_back(new MincStopExpr(getloc(@2, @2))); }
...
void yy::GoParser::error(const location_type &l, const std::string &err_message)
{
...
extern "C"
{
	MincBlockExpr* parseGoStream(std::istream& stream)
	{
		...
		GoLexer lexer(stream, std::cout);
		yy::GoParser parser(lexer, "", &rootBlock);
		parser.parse();
	...
	MincBlockExpr* parseGoFile(const char* filename)
	{
		...
		MincBlockExpr* rootBlock;
		GoLexer lexer(in, std::cout);
		yy::GoParser parser(lexer, filename, &rootBlock);
		parser.parse();
	...
	MincBlockExpr* parseGoCode(const char* code)
	{
		...
		// Parse file into rootBlock
		MincBlockExpr* rootBlock;
		GoLexer lexer(ss, std::cout);
		yy::GoParser parser(lexer, nullptr, &rootBlock);
		parser.parse();
	...
	const std::vector<MincExpr*> parseGoTplt(const char* tpltStr)
	{
		...
		yy::GoParser parser(lexer, nullptr, &tpltBlock);
		if (parser.parse())
			throw CompileError("error parsing template " + std::string(tpltStr));

		// Remove appended STOP expr if last expr is `$B` or `expr ':'`
		assert(tpltBlock->exprs->empty() || tpltBlock->exprs->back()->exprtype == MincExpr::ExprType::STOP);
