...
int main()
{
    ...
    StructType* blockASTType = StructType::create(TheContext, "class.BlockAST");
    StructType* driverType = StructType::create(TheContext, "class.Driver");
    StructType* statementRegisterType = StructType::create(TheContext, "class.StatementRegister");
    StructType* stmtListType = StructType::create(TheContext, "class.std::__cxx11::list");
    StructType* scopeMapType = StructType::create(TheContext, "class.std::map");
    blockASTType->setBody(driverType->getPointerTo(), blockASTType->getPointerTo(), statementRegisterType, stmtListType->getPointerTo(), scopeMapType);
    driverType->setBody(blockASTType->getPointerTo(), blockASTType->getPointerTo());

    std::vector<Type *> FooArgTypes = { blockASTType };
    FT = FunctionType::get(Type::getInt32Ty(TheContext), FooArgTypes, true);
    Function *foo = Function::Create(FT, Function::ExternalLinkage, "foo", TheModule.get());
    printf->setDSOLocal(true);
    verifyFunction(*printf);
